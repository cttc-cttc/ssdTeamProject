...
export default function WishStudy() {
  ...
  useEffect(() => {
    const fetchWishStudies = async () => {
      ...
      try {
        // 백엔드 API 호출
        const response = await axios.get(`${API_BASE}/api/wish/list`, {
        ...
        console.error("위시 스터디 조회 실패:", error);
        // 에러 시 빈 배열로 설정
        setStudies([]);
        setTotalPages(1);
      } finally {
        ...
        {visibleStudies.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-8">
            {visibleStudies.map(study => (
              <Link
                key={study.id}
                to={`/my-page/wish-study/${study.id}/study-detail`}
