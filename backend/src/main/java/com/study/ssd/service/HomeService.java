...
public class HomeService {
    ...
    public Page<StudyPostResponse> getStudyListStudySections(String studySections, int page, int size) {
        ...
        Page<StudyPost> posts = homeRepository.findAllByIsEndedFalse(pageable);
    ...
    public Page<StudyPostResponse> getStudyList(String category, Pageable pageable) {
        ...
        if (category.equals("all")) {
            return homeRepository.findAllByIsEndedFalseOrderByIdDesc(pageable)
                    .map(StudyPostResponse::fromEntity);
        }

        // all 이 아니면 해당 카테고리로 조회
        return homeRepository.findByMainCategoryAndIsEndedFalseOrderByIdDesc(category, pageable)
                .map(StudyPostResponse::fromEntity);
    }
    ...
    public Page<StudyPostResponse> getStudyListByKeyword(String category, String keyword, Pageable pageable) {
        ...
        if (category.equals("all")) {
            return homeRepository.findByTitleContainingIgnoreCaseAndIsEndedFalse(keyword, pageable)
                    .map(StudyPostResponse::fromEntity);
        }

        // all 이 아니면 해당 카테고리에서 검색 키워드로 조회
        return homeRepository.findByMainCategoryAndTitleContainingIgnoreCaseAndIsEndedFalse(category, keyword, pageable)
                .map(StudyPostResponse::fromEntity);
    }
